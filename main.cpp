...
struct Game {
    ...
    void gameOverScreen(Resources& resources, int score, Color startColor, int best_time, bool new_record);
    ...
    struct PlayingScreenData {
        ...
        static constexpr const char* label_t3 = "Backspace para sair";
    };
...
void Game::playingScreen(Resources& resources, bool change, bool toggle, Field field, int points)  {
    ...
    DrawTexture(shovel_sprite, Utils::centralize(48, {GameConstants::windowXPieces[1], GameConstants::windowXPieces[4]}), GameConstants::windowYPieces[9], WHITE);
    DrawTexture(flag_sprite, Utils::centralize(48, {GameConstants::windowXPieces[3], GameConstants::windowXPieces[9]}), GameConstants::windowYPieces[9], WHITE);
        
    DrawText(Game::PlayingScreenData::label_t1, GameConstants::windowXPieces[3], GameConstants::windowYPieces[11], FontSize::body, WHITE);
    DrawText(Game::PlayingScreenData::label_t2, GameConstants::windowXPieces[7], GameConstants::windowYPieces[11], FontSize::body, WHITE);
}

void Game::gameOverScreen(Resources& resources, int score, Color startColor, int best_time, bool new_record) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (game.currentState == GameState::MainTitleScreen) {
            ...
            if (IsKeyPressed(KEY_BACKSPACE)) {
                game.currentState = GameState::SelectionScreen;
                PlaySound(resources->sounds.at("snd_escaped"));
            }
            if (game.field.explosion == true) {
                game.currentState = GameState::GameOverScreen;
            }
            if (game.field.viewSpaces >= game.field.totalSpaces) {
                PlaySound(resources->sounds.at("dumbvictory_sound"));
                game.currentState = GameState::WinningScreen;
            }
